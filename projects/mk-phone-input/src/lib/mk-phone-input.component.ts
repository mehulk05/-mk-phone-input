...
@Component({
  ...
  selector: 'lib-mk-phone-input',
  templateUrl: './mk-phone-input.component.html', 
  styleUrl: './/mk-phone-input.component.css',
  imports: [ MatInputModule,
      ...
      ReactiveFormsModule],
  providers: [
    { provide: MatFormFieldControl, useExisting: MkPhoneInputComponent },
  ],
})
...
export class MkPhoneInputComponent implements OnInit,
AfterViewInit,
DoCheck,
OnDestroy,
...
id = `ngx-mat-tel-input-${MkPhoneInputComponent.nextId++}`;
placeholder: string = '';
focused = false;
errorState = false;
...
/**
 * MkPhoneInputComponent properties and methods
 */
...
onSelectionChange(selection: Country): void {
  this.placeholder = MkPhoneInputComponent.getExampleNumber(selection);
...
private filter(q: string): Countries {
  return this.countries.filter((country: Country): boolean => {
    return (
      MkPhoneInputComponent.includes(country.name.common, q) ||
      MkPhoneInputComponent.includes(country.name.official, q)
    );
  });
}
...
ngOnInit(): void {
  ...
  // Set example number
  this.placeholder = MkPhoneInputComponent.getExampleNumber(defaultCountry);
